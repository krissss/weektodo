...
export default {
  ...
  methods: {
    ...
    onDrop(event, to_index) {
      ...
      const list = customLists.splice(Number.parseInt(from_index), 1)[0]
...
<template>
  <div id="ReorderCustomListsModal" class="modal fade" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">
            {{ $t('ui.reorderCustomLists') }}
          </h5>
            ...
            <li v-for="(list, index) in customTodoLists" :key="index" class="custom-list">
              <div draggable="true" @dragstart="startDrag($event, index)" @dragover.prevent>
                <div class="d-flex flex-row align-items-center">
                  <label
                    class="form-check-label mx-3" @dragenter.self="onDragenter($event)"
                    @dragleave.self="onDragleave($event)" @drop="onDrop($event, index)" @dragover.prevent
                  >{{
                    list.listName
                  }}</label>
                  <i class="bi-grip-horizontal mx-3" :title="$t('ui.remove')" />
                </div>
              </div>
            </li>
          </ul>
