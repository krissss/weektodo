...
export default {
  ...
  watch: {
    repeatingEvent(newVal) {
      const re = this.$store.getters.repeatingEventList[newVal]
  ...
  methods: {
      ...
      let repeatingEventId = this.repeatingEvent ? this.repeatingEvent : moment().format('x')
      if (rule) {
        const date = this.todo.listId
        let re_by_date = this.$store.getters.repeatingEventByDate[date]
        if (!re_by_date)
          re_by_date = {}
      ...
      const reDropDown = document.getElementById('reDropDown')
      const dropdown = new Dropdown(reDropDown)
      dropdown.hide()
    ...
    repeatingEventRule() {
      ...
      if (this.repeatingType == 6) {
        const daysOfMonthArray = this.daysOfMonth.split(',').map(num => Number.parseInt(num))
    ...
    generateRepeatingEvent(rule, repeatingEventId) {
      const todo_data = JSON.parse(JSON.stringify(this.todo))
...
<template>
  ...
  >
    <i id="btnRepeatingEvent" :class="{ 'bi-arrow-clockwise': !repeatingEvent, 'bi-arrow-repeat': repeatingEvent }" />
  </div>
    ...
    <div class="mx-3 d-flex flex-column drop-container">
      ...
      <div v-if="repeatingType" class="d-flex flex-column">
        <div v-show="repeatingType == '5' || (repeatingEvent && repeatingType == '2')" class="weekDays-selector mt-3 mb-1">
          <input id="weekday-mon" v-model="weekdays.mon" type="checkbox" :disabled="repeatingEvent" class="weekday">
          <label for="weekday-mon">{{ moments().isoWeekday(1).locale(language).format("dd")[0] }}</label>
          <input id="weekday-tue" v-model="weekdays.tue" type="checkbox" :disabled="repeatingEvent" class="weekday">
          <label for="weekday-tue">{{ moments().isoWeekday(2).locale(language).format("dd")[0] }}</label>
          <input id="weekday-wed" v-model="weekdays.wed" type="checkbox" :disabled="repeatingEvent" class="weekday">
          <label for="weekday-wed">{{ moments().isoWeekday(3).locale(language).format("dd")[0] }}</label>
          <input id="weekday-thu" v-model="weekdays.thu" type="checkbox" :disabled="repeatingEvent" class="weekday">
          <label for="weekday-thu">{{ moments().isoWeekday(4).locale(language).format("dd")[0] }}</label>
          <input id="weekday-fri" v-model="weekdays.fri" type="checkbox" :disabled="repeatingEvent" class="weekday">
          <label for="weekday-fri">{{ moments().isoWeekday(5).locale(language).format("dd")[0] }}</label>
          <input id="weekday-sat" v-model="weekdays.sat" type="checkbox" :disabled="repeatingEvent" class="weekday">
          <label for="weekday-sat">{{ moments().isoWeekday(6).locale(language).format("dd")[0] }}</label>
          <input id="weekday-sun" v-model="weekdays.sun" type="checkbox" :disabled="repeatingEvent" class="weekday">
        ...
        <div class="d-flex align-items-center justify-content-between re-input">
          ...
          <input v-model="interval" type="number" class="form-control lex-shrink-1 counter" :disabled="repeatingEvent">
        </div>
        ...
        >
          <option value="">
            {{ $t("todoDetails.indefinitely") }}
          </option>
          <option value="ocurrences">
            {{ $t("todoDetails.occurrences") }}
          </option>
          <option value="untilDate">
            {{ $t("todoDetails.untilDate") }}
          </option>
        </select>
